  ...
  return (
    <main className="race-detail-page">
      ...
      <div className="detail-top">
        ...
        <span>RACE DETAIL / {race.date.slice(0, 4)}</span>
      </div>
      <header className="detail-hero">
        <div className="detail-title">
          <div>
            <span className={`status-badge status-${race.status}`}>{race.status}</span>
            <span className="detail-region">{shortRegionName(race.region)}</span>
          </div>
          <h1>{race.대회명}</h1>
          <p>{race.소개내용 || '대회의 주요 일정과 참가 정보를 확인하세요.'}</p>
        </div>
        <div className="detail-date">
          ...
          <small>
            {new Date(`${race.date}T00:00:00`).toLocaleDateString('ko-KR', { weekday: 'long' })}
          </small>
        </div>
      </header>
      <div className="detail-grid">
        <section className="detail-main">
          <article className="detail-facts">
            <h2>대회 기본 정보</h2>
            <dl>
              <div>
                <dt>
                ...
                <dd>{formatKoreanDate(race.date)}</dd>
              </div>
              <div>
                <dt>
                  <Clock3 />
          ...
          <article className="editorial-block">
            <span className="eyebrow">RUNFOR NOTE</span>
            <h2>러너를 위한 코스 노트</h2>
            {race.extra ? (
              <div className="note-grid">
                <div>
                  <span>코스</span>
                  <p>{race.extra.코스설명 || '상세 코스 정보는 공식 요강을 확인해 주세요.'}</p>
                </div>
                <div>
                  <span>난이도</span>
                  <strong>{race.extra.난이도 || '정보 준비 중'}</strong>
                </div>
                <div className="note-tip">
                  <span>
                    <Sparkles size={15} /> 준비 팁
                  </span>
                  <p>{race.extra.팁 || '대회 전날 장비와 이동 동선을 미리 확인하세요.'}</p>
                </div>
              </div>
            ) : (
              <div className="note-empty">고유 코스 리뷰와 준비 팁을 작성 중입니다.</div>
            )}
          </article>
          <article className="registration-info">
            <h2>접수 안내</h2>
            <div>
              <span>접수 기간</span>
              <strong>{race.접수기간}</strong>
            </div>
            <p>
              <TriangleAlert size={16} /> 일정, 코스, 참가비는 주최 측 사정으로 변경될 수 있습니다.
              신청 전 공식 정보를 확인하세요.
            </p>
          </article>
        </section>
        <aside className="detail-aside">
          <div className="apply-card">
            <span className="eyebrow light">ENTRY INFORMATION</span>
            <h2>
              출발선에 설 준비가
              <br />
              되셨나요?
            ...
            <ul>
              <li>
                <Check /> 접수 상태 <strong>{race.status}</strong>
              </li>
              <li>
                <Check /> 참가 종목 <strong>{race.종목.length}개</strong>
              </li>
              <li>
                <Check /> 개최 지역 <strong>{shortRegionName(race.region)}</strong>
              </li>
            </ul>
            {isSample ? (
              <button disabled>공식 링크 준비 중</button>
            ) : (
              <a href={officialLink} target="_blank" rel="nofollow noopener">
                공식 페이지에서 신청 <ArrowUpRight size={17} />
              </a>
            )}
            <small>외부 주최 측 사이트로 이동합니다.</small>
          </div>
          {(race.문의전화 || race.문의이메일) && (
            <div className="contact-card">
              <h3>주최 측 문의</h3>
              {race.문의전화 && (
                <a href={`tel:${race.문의전화}`}>
                  <Phone />
                  {race.문의전화}
                </a>
              )}
              {race.문의이메일 && (
                <a href={`mailto:${race.문의이메일}`}>
                  <Mail />
                  {race.문의이메일}
                </a>
              )}
