...
- FullCalendar 월간 대회 달력 (기본값: 오늘이 속한 달)
- 대회 일정 요약 팝업
- 반응형 대회 목록
- 키워드·지역·접수 상태·개최 월 필터
- 지도·달력·목록 필터 실시간 연동
...
- 실제 수집 대회 데이터 993건 연동 (marathongo 스키마, `lib/races.ts`에서 변환)
- YouTube 추천 채널 6개 연동

### 운영 전에 남은 작업

- 대회별 직접 작성 콘텐츠(`race-extra.json`) 확충 — 현재 6건만 작성됨
- `지역` 값이 표준 17개 시·도 별칭과 안 맞는 대회 소수 정리 (예: "음성", "보성", "강릉" 등 시·군명이 들어간 항목)
- `종목배열`의 자유 텍스트(예: "10km 커플런", "100Miles") 표준화 여부 검토
- 개인정보처리방침 문구 실제 서비스에 맞춰 법률 검토 (운영 이메일은 `runfor365@gmail.com`으로 설정됨)
- Google Search Console 및 웹 분석 도구 연결
- 콘텐츠가 충분히 쌓인 뒤 AdSense 신청
- `runfor.kr`, `run4.kr` 도메인 등록 및 연결
- 정기 재수집용 Tampermonkey 스크립트 제작 또는 검증 (현재는 1회성 수집 데이터 사용 중)
...
현재 데이터는 `marathongo.co.kr` 형식으로 수집된 원본이며, `types/race.ts`의 `RawRace` 타입이 이 구조를 그대로 따릅니다. 각 대회는 다음 구조를 사용합니다.

```json
{
  "id": 486,
  "대회명": "버닝런 일레븐",
  "slug": "burning-run-eleven-2025-11-30",
  "날짜": "2025-11-30",
  "집결": "09:00 집결",
  "종목": "10km,7km,5km",
  "종목배열": ["10km", "7km", "5km"],
  ...
  "접수마감": "2025-11-03",
  "접수상태": "접수마감",
  "홈페이지": "http://www.burningrun.co.kr",
  "이메일": "run@runandrun3.com",
  "전화": "070-7706-1011",
  "소개내용": "대회 소개 내용",
  "품절여부": null,
  "중단여부": null,
  "상세링크": "https://marathongo.co.kr/raceDetail/domestic/burning-run-eleven-2025-11-30",
  "이미지": "https://marathongo.co.kr/assets/image/race/domestic/burning-run-eleven-2025-11-30.webp"
}
```

`data/races.json`은 이 원본 스키마를 그대로 유지합니다. 화면과 상세 페이지가 실제로 쓰는 필드명(`장소상세`, `집결시간`, `접수기간`, `신청링크`, `문의이메일` 등)과는 다르기 때문에, **`lib/races.ts`의 `getRaces()`가 두 스키마를 변환**합니다.

| 원본 필드 (`RawRace`) | 변환 후 필드 (`NormalizedRace`) | 비고 |
| --- | --- | --- |
| `종목배열` | `종목` | 중복 값은 `Set`으로 제거 |
| `장소` | `장소상세` | 상세 주소 필드가 따로 없어 동일 값 사용 |
| `집결` | `집결시간`, `집결정보` | |
| `접수시작` + `접수마감` | `접수기간` | `"YYYY-MM-DD ~ YYYY-MM-DD"` 형태로 결합 |
| `홈페이지` (없으면 `상세링크`) | `신청링크` | |
| `이메일` / `전화` | `문의이메일` / `문의전화` | |
| `날짜` (이미 `YYYY-MM-DD`) | `date` | 형식이 아니면 `{올해}-01-01`로 폴백 |
| `지역` | `region` | `lib/region-map.ts`로 정규화 (9.6 참고) |
| `접수상태` | `status` | `lib/date.ts`의 `normalizeStatus`로 `접수중/접수예정/접수마감` 3종으로 정규화. `"품절"`, `"중단"` 문자열도 `접수마감`으로 처리 |

새 수집 스크립트를 다른 사이트 기준으로 만들 경우, 원본 필드명이 달라질 수 있습니다. 그때는 `data/races.json`을 억지로 지금 스키마에 맞추기보다 **`lib/races.ts`의 매핑 로직을 새 스키마에 맞게 고치는 쪽**을 권장합니다. (원본 데이터를 있는 그대로 보존해야 재수집·검증이 쉽습니다.)
...
| `날짜` | `YYYY-MM-DD` 형식. 캘린더/정렬/상세 페이지가 이 값을 그대로 사용 |
| `종목배열` | 문자열 배열. 자유 텍스트 허용되지만 같은 대회 안에서 중복 값은 자동 제거됨 |
| `지역` | 17개 시·도 별칭(9.6 참고) 중 하나와 일치해야 지도에 집계됨 |
| `장소` | 목록·상세에 표시할 장소 |
| `집결` | 목록·상세에 표시할 집결 시간 안내 |
| `접수상태` | `접수중`, `접수예정`, `접수마감`, `품절`, `중단` 등 — `normalizeStatus`가 정규화 |
| `접수시작`, `접수마감` | 접수 기간 표시에 결합되어 사용 |
| `상세링크` | 외부 대회 정보 링크. `홈페이지`가 없을 때 신청 링크로도 사용됨 |

### 9.3 선택 필드

- `홈페이지`: 있으면 상세 페이지의 신청 버튼 링크로 우선 사용
- `이메일`, `전화`: 주최 측 문의처로 표시
- `소개내용`: 대회 소개
- `이미지`: 향후 이미지 기능 확장용
- `품절여부`, `중단여부`: 현재 화면 로직에서는 사용하지 않음 (참고용 원본 값)
...
`날짜` 필드가 `YYYY-MM-DD` 형식이면 그대로 사용합니다. 형식이 아니면 `lib/races.ts`가 `{현재 연도}-01-01`로 폴백하므로, 수집 단계에서 반드시 ISO 형식으로 정리해야 합니다.

### 9.6 지도 지역 정규화

지도 지역은 `지역` 필드를 `lib/region-map.ts`의 `normalizeRegion()`으로 변환해 사용합니다.
...
별칭은 `lib/region-map.ts`의 `aliases` 객체에서 관리합니다. `지역` 값이 시·군명(예: "음성", "강릉")이거나 별칭 목록에 없으면 지도 색상 집계에서는 빠지고, 목록·달력에는 정상적으로 노출됩니다. 새로운 표기가 들어오면 `aliases`에 추가합니다.

### 9.7 실제 JSON으로 교체하는 순서

전체 절차는 19번 "정기 운영 절차"를 따릅니다. 요약하면: JSON 문법·slug 중복·날짜 형식을 확인하고 → `data/races.json` 교체 → `npm run lint && npm run build`로 검증 → 로컬 확인 → Git commit/push → Vercel Preview 확인 순서입니다.

### 9.8 데이터 교체 시 주의

- `상세링크`나 `홈페이지`에 `example.com` 같은 테스트 링크가 남아있으면 상세 페이지 신청 버튼이 비활성화됩니다.
- `종목배열`은 자유 텍스트 원본을 그대로 씁니다. 표준화하지 않아도 화면 표시는 문제없지만, 정확한 거리 기준 검색 기능을 추가하려면 별도 정규화가 필요합니다.
...
Tampermonkey 스크립트는 방문자 기능이 아니라 관리자 전용 데이터 수집 도구입니다. 현재 이 저장소에는 수집 스크립트가 포함되어 있지 않으며, `data/races.json`은 1회성으로 수집한 데이터를 그대로 사용 중입니다. 정기적으로 재수집하려면 별도 제작·검증이 필요합니다.
...
9.1~9.3에 정리된 `RawRace` 필드(대회명, slug, 날짜, 집결, 종목배열, 지역, 장소, 접수시작/접수마감, 접수상태, 홈페이지, 이메일, 전화, 소개내용, 상세링크, 이미지 등)와 동일한 구조로 내보내면 코드 수정 없이 바로 반영됩니다. 원본 사이트의 필드명이 다르면 `data/races.json`을 억지로 맞추지 말고 `lib/races.ts`의 매핑 로직을 새 스키마에 맞춰 고치는 편이 안전합니다 (9.1 참고).
...
```text
https://배포주소/
https://배포주소/race/{data/races.json의 slug 하나}
https://배포주소/api/news
...
- `지역` 값을 확인합니다.
- `lib/region-map.ts`의 `aliases`에 별칭이 있는지 확인합니다.
- 예: `경북`이 아니라 시·군명(예: "영주") 같은 새로운 표기가 들어왔다면 aliases에 추가합니다.

### 달력에 대회가 표시되지 않을 때

- `날짜`가 `YYYY-MM-DD` 형식인지 확인합니다.
...
- **배포 플랫폼:** Vercel 예정 (아직 도메인 미연결)
- **현재 상태:** 실제 수집 데이터 993건으로 전환 완료, 로컬 프로덕션 빌드 검증 완료
- **검증 환경:** Windows, Node.js
- **ESLint:** 통과
- **TypeScript:** 통과
- **Next.js 프로덕션 빌드:** 통과
- **정적·SSG 경로:** 메인, 소개, 정책, 아이콘, 사이트맵, robots, 대회 상세 993개 생성 확인
- **뉴스 API:** RSS 항목 응답 확인
- **YouTube API:** 채널 6개 등록, 실제 영상 응답 확인
- **브라우저 콘솔:** 오류 없음 (경북 지역 종목 중복 key 오류는 수정 완료)
